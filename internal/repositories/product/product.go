...
func (r *productRepository) FilterAndSortProducts(size string, minPrice, maxPrice float64, color string, categoryID uint) ([]models.Product, error) {
	var products []models.Product
	query := r.db.Preload("ProductDetails").Preload("OrderDetails").Where("1=1")

	if size != "" {
		query = query.Joins("JOIN product_details ON product_details.product_id = products.id").Where("product_details.size = ?", size)
	}
	if minPrice >= 0 {
	...
	if color != "" {
		query = query.Joins("JOIN product_details ON product_details.product_id = products.id").Where("product_details.color = ?", color)
	}
	if categoryID > 0 {
